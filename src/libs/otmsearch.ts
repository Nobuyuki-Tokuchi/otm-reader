import { SearchNode, ValueNode, OperatorNode, StatementNode, PatternNode, KeyNameNode, VariableNode, KeywordVariableNode, StringNode, NumberNode, BinaryOperatorNode, UnaryOperatorNode, BindOperatorNode, MatchingOperatorNode, BraceNode, MatchingType } from "./otmsearch.node";
import { SearchToken, KeyNameToken, VariableToken, KeywordVariableToken, StringToken, NumberToken, BinaryOperatorToken, UnaryOperatorToken, MatchingOperatorToken, BraceToken, PatternBraceToken, StatementBraceToken, CommaToken, BindToken } from "./otmsearch.token";
...
export default class OtmSearch {
    ...
    private parsePattern(tokens: SearchToken[], count: number): [BindOperatorNode | StatementNode | PatternNode, number] {
        ...
        const hasBrace = token instanceof PatternBraceToken && token.tokenValue === "[";
    ...
    private parseStatement(tokens: SearchToken[], count: number): [BindOperatorNode | StatementNode, number] {
        let token = tokens[count];
        const hasBrace = token instanceof StatementBraceToken && token.tokenValue === "{";
        ...
        if (keyName instanceof KeyNameToken) {
            ...
            if (names.length > 1) {
                ...
                for (let i = 1; i < names.length; i++) {
                    const branch = new BindOperatorNode();
    ...
    private parseCompare(tokens: SearchToken[], count: number): [ValueNode | OperatorNode, number] {
        ...
        const token = tokens[count];
        let operator: BinaryOperatorNode;
        ...
        if(token instanceof BinaryOperatorToken) {
            switch(token.operator) {
                ...
                case "<=":
                case ">=":
                    operator = new BinaryOperatorNode(token.operator);
                    operator.lhs = node;
